...
def camera_loop_update_checks(
...
) -> tuple[CameraConfig, np.recarray, VideoRecorder | None, CameraStatus]:
    ...
    frame_rec_array, config = check_for_new_config(
    ...
    self_status = check_camera_should_pause(config=config, ipc=ipc, self_status=self_status)

    return config, frame_rec_array, video_recorder, self_status


def check_camera_should_pause(
    config: CameraConfig,
    ipc: CameraGroupIPC,
    self_status: CameraStatus,
) -> CameraStatus:
